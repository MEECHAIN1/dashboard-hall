...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-slate-300 flex flex-col font-sans selection:bg-indigo-500 selection:text-white">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-3 border-b border-slate-800">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono font-bold text-white text-xs uppercase tracking-wider">
                Phase 3 Production Topology & Security Audit
              </span>
            </div>
            <div className="flex items-center gap-2 text-[11px] font-mono">
              <span className="text-emerald-400 font-bold flex items-center gap-1">
                <Check className="w-3 h-3 text-emerald-400" /> DUAL PROBES: ONLINE
              </span>
              <span className="text-slate-700">•</span>
              <span className="text-slate-400">dashboard.meechain.live</span>
            </div>
          </div>
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="p-3 bg-[#050505] rounded-lg border border-slate-800">
              <span className="text-slate-500 block mb-1 text-[10px] uppercase font-mono tracking-wider">
                api.meechain.live (Live Ingress)
              </span>
              <span className="font-mono text-emerald-300 text-xs">HTTPS / TLS 1.3 Active</span>
            </div>
            <div className="p-3 bg-[#050505] rounded-lg border border-slate-800">
              <span className="text-slate-500 block mb-1 text-[10px] uppercase font-mono tracking-wider">
                rpc.meechain.live (JSON-RPC)
              </span>
              <span className="font-mono text-indigo-300 text-xs">eth_blockNumber Synced</span>
            </div>
            <div className="p-3 bg-[#050505] rounded-lg border border-slate-800">
              <span className="text-slate-500 block mb-1 text-[10px] uppercase font-mono tracking-wider">
                NEXT_PUBLIC_CHAIN_ID
              </span>
              <span className="font-mono text-amber-300 text-xs">13390 (MeeChain Mainnet)</span>
            </div>
            <div className="p-3 bg-[#050505] rounded-lg border border-slate-800">
              <span className="text-slate-500 block mb-1 text-[10px] uppercase font-mono tracking-wider">
                Definition of Done (DoD)
              </span>
              <span className="font-mono text-emerald-400 font-bold flex items-center gap-1 text-xs">
                8/8 Tests Verified 🟢
              </span>
            </div>
          </div>
      ...
      {/* Footer matching Geometric Balance & Phase 3 spec */}
      <footer className="mt-8 border-t border-slate-800 bg-[#050505] py-4 text-xs text-slate-400">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-4">
            <span className="text-[10px] text-slate-500 uppercase tracking-widest font-mono">Philosophy:</span>
            <span className="text-xs text-slate-300 font-serif italic">"Bridge over walls — decentralized balance"</span>
          </div>
          <div className="flex gap-4 text-[10px] text-slate-500 font-mono">
            <span>SHA-256: 0x9f8...a12</span>
            <span className="text-emerald-400 font-bold">V.3.0.0-PROD-VERIFIED 🟢</span>
          </div>
        </div>
      </footer>
    </div>
