export interface ProductionFileMeta {
  ...
  language: string;
  targetPath: string; // Real path in repo — server uses this to query GitHub exclusively
  description: string;
...
export const PRODUCTION_FILE_REGISTRY: ProductionFileMeta[] = [
  ...
  },
  {
    ...
    description: 'Live probe of API Gateway and RPC Node with source/identity separation',
  },
  {
    ...
    description: 'Relay log GET/POST connected to Supabase edge function',
  },
  {
    id: 'stats-route',
    name: 'stats/route.ts',
...
export function getRegistryEntry(id: string): ProductionFileMeta | null {
